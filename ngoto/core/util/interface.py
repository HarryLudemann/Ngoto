...
def tasks(tasks: list, thisOS: str, logger):
    ...
    for task in tasks:
        if thisOS in task.os:
            table.add_row(task.id, str(task.delay), task.description, task.last_output, str(task.iteration), str(task.active), str(task.os))
        else:
            table.add_row(task.id, str(task.delay), task.description, task.last_output, str(task.iteration), "False", str(task.os), style=greyed_out_style)
    output(table)
